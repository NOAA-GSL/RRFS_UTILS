...
## Dependencies external to lightning.fd

- MPI library
- NetCDF-Fortran library
- NCEPLIBS-bufr library
- NCEPLIBS-w3nco library
- `module_kinds.f90`
- `module_mpasio.f90`
- `module_map_proj_helper.f90`
- `module_wps_map_utils.f90`
...
| `search_rad` | 1 | How far to search around a map projection point containing lightning observations for an MPAS cell center. Units are grid boxes of map projection (so distance in meters = `(search_rad) X (grid spacing of map projection)`). Recommended value is `(MPAS mesh spacing) / (map projection grid spacing)`, rounded up to the nearest integer. |
